...
MIXAMO_DATASET_ROOT = "D:\Datasets\mixamo" # Change this to your data directory
assert MIXAMO_DATASET_ROOT is not None, "Update the location of the SRN Shapenet Dataset"
...
class MixamoDataset(Dataset):
    def __init__(self, cfg,
        ...
        self.base_path = os.path.join(MIXAMO_DATASET_ROOT, "mixamo_{}".format(dataset_name))

        self.intrins = sorted(
            glob.glob(os.path.join(self.base_path, "*", "intrinsics.json"))
        )
    ...
    def load_example_id(self, example_id, intrin_path,
        ...
        rgb_paths = sorted(glob.glob(os.path.join(dir_path, "img", "*")))
        ...
        if example_id not in self.all_rgbs.keys():
            ...
            cam_infos = readCamerasFromTxt(rgb_paths, pose_paths, [i for i in range(len(rgb_paths))], flip=True)
